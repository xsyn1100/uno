...
class Game(object):
    ...
    def turn(self):
        ...
        self.logger.debug("Sonraki Oyuncu")
    ...
    def play_card(self, card):
        ...
        self.logger.info("Oyun kartları " + repr(card))
        if card.value == c.SKIP:
            self.turn()
        elif card.special == c.DRAW_FOUR:
            self.draw_counter += 4
            self.logger.debug("Beraberlik sayacı 4 arttı")
        elif card.value == c.DRAW_TWO:
            self.draw_counter += 2
            self.logger.debug("Beraberlik sayacı 2 arttı")
