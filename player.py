...
class Player(object):
    ...
    def _card_playable(self, card):
        ...
        is_playable = True
        last = self.game.last_card
        self.logger.debug("Kontrol kartı " + str(card))

        if (card.color != last.color and card.value != last.value and
                not card.special):
            self.logger.debug("Kartın rengi veya değeri eşleşmiyor!")
            is_playable = False
        elif last.value == c.DRAW_TWO and not \
                card.value == c.DRAW_TWO and self.game.draw_counter:
            self.logger.debug("Oyuncu çizmek zorunda ve karşı koyamıyor")
            is_playable = False
            ...
            self.logger.debug("Diğerinde renk seçici oynatılamıyor")
            is_playable = False
        elif not last.color:
            self.logger.debug("Son kartın rengi yok")
            is_playable = False
